/*****************************************************************************\
 *  aes.c - implementation-independent job of influxdb info
 *  functions
 *****************************************************************************/
#include "aes.h"
#include <string.h>
...
int key_data_encryption(const uint8_t *key, uint32_t keyLen, AesKey *aesKey) {
    ...
    if (NULL == key || NULL == aesKey){
        printf("key_data_encryption param is NULL\n");
        return -1;
    }

    if (keyLen != 16){
        printf("key_data_encryption keyLen = %d, Not support.\n", keyLen);
        return -1;
    }
...
int mix_cols(uint8_t (*state)[4]) {
    ...
    for ( i = 0; i < 4; ++i) {
        for ( j = 0; j < 4; ++j) { 
            ...
            state[i][j] = galois_multiply(M[i][0], tmp[0][j]) ^ galois_multiply(M[i][1], tmp[1][j])
                        ^ galois_multiply(M[i][2], tmp[2][j]) ^ galois_multiply(M[i][3], tmp[3][j]);
        }
    }
...
int encrypt_aes(const uint8_t *key, uint32_t keyLen, const uint8_t *pt, uint8_t *ct, uint32_t len) {
    ...
    /*Key expansion*/
    key_data_encryption(actualKey, 16, &aesKey);  
    ...
    for ( i = 0; i < len; i += BLOCKSIZE) {
        ...
        load_state_array(state, pt);

        round_key_add(state, rk);
...
extern int decrypt_aes(const uint8_t *key, uint32_t keyLen, const uint8_t *ct, uint8_t *pt, uint32_t len) {
    ...
    for ( i = 0; i < len; i += BLOCKSIZE) {
        ...
        load_state_array(state, ct);
        /*Round secret key addition, same as encryption*/ 
        round_key_add(state, rk);

        for ( j = 1; j < 10; ++j) {
            rk += 4;
            reverse_shift_rows(state);   
            apply_inv_sbox(state);     
            round_key_add(state, rk); 
            inv_column_mix(state);   
        }
