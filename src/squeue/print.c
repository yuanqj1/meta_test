...
#ifdef __METASTACK_OPT_PRINT_COMMAND
		char temp[width + 1];
		if((length > width) && right_format &&(length > 0)) {
			strncpy(temp,str + (length - width),width);
			temp[width] = '\0';
...
{
	if (job == NULL)
		_print_str("ORDER", width, right_justify, true);
	else
		_print_int(job->pending_order, width, right_justify, true);
	if (suffix)
		printf("%s", suffix);
	return SLURM_SUCCESS;
}
