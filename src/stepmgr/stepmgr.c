...
pthread_mutex_t agent_msg_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static step_record_t *_build_interactive_step(
	...
	job_step_create_request_msg_t *step_specs,
	uint16_t protocol_version, watch_dog_record_t *watch_dog_ptr);
...
#ifdef __METASTACK_BUG_SRUN_STEPNODE_ERR
static int _job_req_nodes_num(const char *step_node_list)
{
	hostlist_t *step_hostlist = NULL;
	int step_count = -1;

	if (!step_node_list)
		return step_count;

	/* Create hostlist for step node lists */
	step_hostlist = hostlist_create(step_node_list);

	if (step_hostlist){
		/* Get node counts */
		step_count = hostlist_count(step_hostlist);
		hostlist_destroy(step_hostlist);
	}
	return step_count;
}
#endif
...
/*
 * create_step_record_no_list - create an empty step_record for the specified job and do not add it to the step_list.
 * IN job_ptr - pointer to job table entry to have step record added
 * IN protocol_version - slurm protocol version of client
 * RET a pointer to the record or NULL if error
 * NOTE: allocates memory that should be xfreed with delete_step_record
 */
static step_record_t *create_step_record_no_list(job_record_t *job_ptr,
				...
				job_step_create_request_msg_t *step_specs)
{
	...
	if (step_pendlist_dedup) {
		step_ptr = create_step_record_no_list(job_ptr, 0);
	} else {	
	...
	if((msg_type == REQUEST_SIGNAL_TASKS) && step_ptr){
		...
		*/
		...
		if (signal_job_list == NULL){
			...
			list_itr_t *itr = list_iterator_create(signal_job_list);
			while((signal_job_info = list_next(itr))){
				  ...
				  int *return_code)
{
...
#ifdef __METASTACK_BUG_SRUN_STEPNODE_ERR
	bitstr_t *effective_idle = NULL;
	bitstr_t *combined_nodes = NULL;
#endif
	...
	*return_code = SLURM_SUCCESS;
	 ...
	 * Fix bug 98880
	 * When calling srun in sbatch, fix the problem that the nodes requested 
	 * by mpirun in openmpi may be inconsistent with the nodes allocated
	 */
...
#ifdef __METASTACK_BUG_SRUN_STEPNODE_ERR
	} else if (step_spec->node_list && !xstrcmp(step_spec->node_list,
                        job_ptr->details->req_nodes)) {
		log_flag(STEPS, "%s: step_spec nodelist %s and job_ptr->details->req_nodes is equal.", __func__, step_spec->node_list);
		int req_count = _job_req_nodes_num(step_spec->node_list);
		if (req_count > 0) {
			if (req_count >= bit_set_count(job_ptr->node_bitmap)) {
				nodes_picked = bit_alloc(bit_size(nodes_avail));
			} else {
                bitstr_t *selected_nodes = NULL;
                log_flag(STEPS, "%s: selected nodelist is %s",
	...
	if (step_spec->relative != NO_VAL16) {
		...
		bit_not(nodes_idle);
#ifdef __METASTACK_BUG_SRUN_STEPNODE_ERR
		effective_idle = bit_copy(nodes_idle);
#endif
	...
	if ((step_spec->num_tasks == NO_VAL) &&
		...
		if ((step_spec->flags & SSF_OVERLAP_FORCE) && nodes_avail) {
			...
			if (nodes_avail && nodes_picked) {
				combined_nodes = bit_copy(nodes_avail);
				bit_or(combined_nodes, nodes_picked);
			} else if (nodes_avail) {
				combined_nodes = bit_copy(nodes_avail);
			} else if (nodes_picked) {
				combined_nodes = bit_copy(nodes_picked);
			}
			if (combined_nodes && effective_idle) {
				bit_and(effective_idle, combined_nodes);
				cnt = bit_set_count(effective_idle);
				node_bitmap = effective_idle;
			} else if (nodes_idle) {
...
#ifdef __METASTACK_BUG_SRUN_STEPNODE_ERR
	FREE_NULL_BITMAP(combined_nodes);
	FREE_NULL_BITMAP(effective_idle);
#endif
	FREE_NULL_BITMAP(nodes_avail);
	FREE_NULL_BITMAP(select_nodes_avail);
...
#endif
{
	...
	*new_step_record = NULL;
	...
	if (step_specs->flags & SSF_INTERACTIVE) {
		...
		*new_step_record = _build_interactive_step(job_ptr, step_specs,
							   protocol_version, watch_dog_ptr);
#endif
		if (*new_step_record)
			return SLURM_SUCCESS;
		else
	...
	if (step_specs->flags & SSF_EXT_LAUNCHER) {
		...
		return _build_ext_launcher_step(new_step_record, job_ptr,
						step_specs, protocol_version, watch_dog_ptr);
#endif
	}
	...
	*stepmgr_ops->last_job_update = time(NULL);
	...
	} else if(job_ptr->details->watch_dog) {
...
#ifdef __METASTACK_BUG_CR_CORE
		if ((step_ptr->pn_min_memory & MEM_PER_CPU) && _is_mem_resv(job_ptr)) {
#else
...
#ifdef __METASTACK_BUG_CR_CORE
		} else if ((!step_ptr->pn_min_memory) && _is_mem_resv(job_ptr)) {
#else
...
#endif
{
	...
	error_code = step_create(job_ptr, req_step_msg, &step_rec,
				 msg->protocol_version, &err_msg, watch_dog_ptr);
#endif
...
end_it:
	...
	if (error_code) {
		...
		if ((job_step_resp.use_protocol_ver >= META_2_0_PROTOCOL_VERSION) &&
		    	(job_step_resp.use_protocol_ver < SLURM_23_02_PROTOCOL_VERSION)) {
