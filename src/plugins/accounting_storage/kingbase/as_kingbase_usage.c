/*****************************************************************************\
 *  as_kingbase_usage.c - functions dealing with usage.
 *****************************************************************************
...
static void *_cluster_rollup_usage(void *arg)
{
	...
	slurmdb_rollup_stats_t *rollup_stats = local_rollup->rollup_stats;
	time_t hour_start;
	time_t hour_end;
	time_t day_start;
	time_t day_end;
	...
	/*
	 * rollup_stats is unique per thread so should be ok to alter
	 * unprotected.
	 */
	...
	rc = check_connection(&kingbase_conn);
	...
	if (!local_rollup->sent_start) {
		...
		if (KCIResultGetRowCount(result)>0) {
			last_hour = slurm_atoul(KCIResultGetColumnValue(result,0,DBD_ROLLUP_HOUR));
			last_day = slurm_atoul(KCIResultGetColumnValue(result,0,DBD_ROLLUP_DAY));
			last_month = slurm_atoul(KCIResultGetColumnValue(result,0,DBD_ROLLUP_MONTH));

			/* only record timestamps if db provided */
			rollup_stats->timestamp[DBD_ROLLUP_HOUR] = last_hour;
			rollup_stats->timestamp[DBD_ROLLUP_DAY] = last_day;
			rollup_stats->timestamp[DBD_ROLLUP_MONTH] = last_month;
		} else {
			...
			DB_DEBUG(DB_USAGE, kingbase_conn.conn, "query\n%s", query);
			//info("[query] line %d, %s: query: %s", __LINE__, __func__, query);
			result = kingbase_db_query_ret(&kingbase_conn, query, 0);
			xfree(query);
			if (KCIResultGetStatusCode(result) != EXECUTE_TUPLES_OK) {
			...
			if (KCIResultGetRowCount(result)>0) {
				time_t check = slurm_atoul(KCIResultGetColumnValue(result,0,0));
				if (check < lowest)
					lowest = check;
			}
...
/* 	info("diff is %d", month_end-month_start); */
	...
	if ((hour_end - hour_start) > 0) {
		START_TIMER;
		rc = as_kingbase_hourly_rollup(&kingbase_conn,
					    local_rollup->cluster_name,
					    hour_start,
					    hour_end,
	...
	if ((day_end - day_start) > 0) {
		START_TIMER;
		rc = as_kingbase_nonhour_rollup(&kingbase_conn, 0,
					     local_rollup->cluster_name,
					     day_start,
					     day_end,
					     local_rollup->archive_data);
	...
	if ((month_end - month_start) > 0) {
		START_TIMER;
		rc = as_kingbase_nonhour_rollup(&kingbase_conn, 1,
					     local_rollup->cluster_name,
					     month_start,
					     month_end,
					     local_rollup->archive_data);
...
static int _get_object_usage(kingbase_conn_t *kingbase_conn,
			     ...
			     time_t start, time_t end, List *usage_list)
{
	...
	for (int j = 0; j < cnt; j++){
		...
		accounting_rec->tres_rec.id = slurm_atoul(KCIResultGetColumnValue(result,j,USAGE_TRES));
		...
		accounting_rec->id = slurm_atoul(KCIResultGetColumnValue(result,j,USAGE_ID));
		accounting_rec->period_start = slurm_atoul(KCIResultGetColumnValue(result,j,USAGE_START));
		accounting_rec->alloc_secs = slurm_atoull(KCIResultGetColumnValue(result,j,USAGE_ALLOC));
...
static int _get_cluster_usage(kingbase_conn_t *kingbase_conn, uid_t uid,
			      ...
			      time_t start, time_t end)
{
	...
	for (int j = 0; j < cnt; j++) {
		...
		accounting_rec->tres_rec.id = slurm_atoul(KCIResultGetColumnValue(result,j,CLUSTER_TRES));
		accounting_rec->tres_rec.count = slurm_atoul(KCIResultGetColumnValue(result,j,CLUSTER_CNT));
		if ((tres_rec = list_find_first(
			     assoc_mgr_tres_list, slurmdb_find_tres_in_list,
			     &accounting_rec->tres_rec.id))) {
			accounting_rec->tres_rec.name =
		...
		accounting_rec->alloc_secs = slurm_atoull(KCIResultGetColumnValue(result,j,CLUSTER_ACPU));
		accounting_rec->down_secs = slurm_atoull(KCIResultGetColumnValue(result,j,CLUSTER_DCPU));
		accounting_rec->pdown_secs = slurm_atoull(KCIResultGetColumnValue(result,j,CLUSTER_PDCPU));
		accounting_rec->idle_secs = slurm_atoull(KCIResultGetColumnValue(result,j,CLUSTER_ICPU));
		accounting_rec->over_secs = slurm_atoull(KCIResultGetColumnValue(result,j,CLUSTER_OCPU));
		accounting_rec->plan_secs = slurm_atoull(KCIResultGetColumnValue(result,j,CLUSTER_PCPU));
		accounting_rec->period_start = slurm_atoul(KCIResultGetColumnValue(result,j,CLUSTER_START));
			      ...
			      char *cluster_name, time_t start, time_t end)
{
	...
	if (_get_object_usage(kingbase_conn, type, my_usage_table, cluster_name,
...
extern int as_kingbase_get_usage(kingbase_conn_t *kingbase_conn, uid_t uid,
			      ...
			      time_t start, time_t end)
{
	...
	case DBD_GET_CLUSTER_USAGE:
		rc = _get_cluster_usage(kingbase_conn, uid, in,
					type, start, end);
		return rc;
		break;
	default:
...
is_user:
	...
	_get_object_usage(kingbase_conn, type, my_usage_table, cluster_name,
...
extern int as_kingbase_roll_usage(kingbase_conn_t *kingbase_conn, time_t sent_start,
			       ...
			       List *rollup_stats_list_in)
{
	...
	*rollup_stats_list_in = list_create(slurmdb_destroy_rollup_stats);
	...
	while ((cluster_name = list_next(itr))) {
		...
		local_rollup->kingbase_conn = kingbase_conn;
		local_rollup->rc = &rc;
		...
		local_rollup->rollup_stats =
			xmalloc(sizeof(slurmdb_rollup_stats_t));
		local_rollup->rollup_stats->cluster_name =
			xstrdup(cluster_name);
		list_append(*rollup_stats_list_in, local_rollup->rollup_stats);
		/* _cluster_rollup_usage is responsible for freeing
		   this local_rollup */
		/* If you have many jobs in your system the
		 * _cluster_rollup_usage exec takes up a bunch of time
		 * and all the while the as_kingbase_cluster_list_lock is
		 ...
		 * as_kingbase_cluster_list_lock lock unlock in a timely
...
extern bool trigger_reroll(kingbase_conn_t *kingbase_conn, time_t event_time)
{
	slurm_mutex_lock(&rollup_lock);
	if (event_time < global_last_rollup) {
		...
		DB_DEBUG(DB_USAGE, kingbase_conn->conn, "query\n%s", query);
		(void) kingbase_db_query(kingbase_conn, query);
		xfree(query);
		return true;
	}
