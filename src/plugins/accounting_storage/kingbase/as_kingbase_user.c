...
static int _foreach_add_user(void *x, void *arg)
{
	...
	KCIResult_t *result = kingbase_db_query_ret(add_user_cond->kingbase_conn, query, 0);
...
#ifdef __METASTACK_OPT_USER_DEACTIVATE
		kingbase_conn, uid, is_deactivate, NULL, &user_coord_cond);
#else
...
#ifdef __METASTACK_OPT_USER_DEACTIVATE
	coord_list = as_kingbase_remove_wckeys(kingbase_conn, uid, is_deactivate, &wckey_cond);
#else
...
#ifdef __METASTACK_OPT_USER_DEACTIVATE
		if (is_deactivate)
			rc = deactivate_common(kingbase_conn, DBD_DEACTIVATE_USERS, now,
						user_name, user_table, name_char,
...
no_user_table:
	...
	if (name_char && vals) {
		xstrcat(name_char, ")");
		user_name = uid_to_string((uid_t) uid);
		rc = activate_common(kingbase_conn, DBD_ACTIVATE_USERS, now,
				   user_name, user_table, name_char,
				   vals, NULL);
		xfree(user_name);
	}
	...
	if (rc == SLURM_SUCCESS) {
		...
		local_user_list = as_kingbase_get_users(kingbase_conn, uid, user_cond);
		if (local_user_list) {
	...
	assoc_cond.user_list = ret_list;
	user_coord_cond.assoc_cond = &assoc_cond;
	coord_list = as_kingbase_activate_coord(kingbase_conn, uid, NULL, &user_coord_cond);
	FREE_NULL_LIST(coord_list);
	...
	if (user_cond->assoc_cond
		...
		wckey_cond.cluster_list =
			user_cond->assoc_cond->cluster_list;
	tmp_list = as_kingbase_activate_wckeys(kingbase_conn, uid,
						&wckey_cond, &wckey);
				  ...
				  slurmdb_user_cond_t *user_cond)
{
	...
	query = xstrdup_printf(
		"select `user`, acct from %s where deleted=%d and %s order by `user`",
	...
	for (int i = 0; i < cnt; i++) {
		...
		query2 = xstrdup_printf( 
			...
			"u.deleted = 0 AND a.deleted = 0;",
			 acct_coord_table, user_table, acct_table, 
			 KCIResultGetColumnValue(result,i,0), KCIResultGetColumnValue(result,i,1));
		result2 = kingbase_db_query_ret(kingbase_conn, query2, 0);
		xfree(query2);
		...
		cnt2 = KCIResultGetRowCount(result2);
		KCIResultDealloc(result2);
		if (!cnt2)
			continue;
		...
		if (set)
			xstrcat(extra, " or ");
		xstrfmtcat(extra, "(`user`='%s' && acct='%s')", KCIResultGetColumnValue(result,i,0), KCIResultGetColumnValue(result,i,1));
		set = 1;
				  ...
				  slurmdb_user_cond_t *user_cond)
{
	...
	if (is_deactivate)
		...
		query = xstrdup_printf(
			"select `user`, acct from %s where (deleted=0 or deleted=%d) and %s order by `user`",
			acct_coord_table, SLURMDB_USER_DEACTIVATED, extra);
#else
	query = xstrdup_printf(
		"select `user`, acct from %s where deleted=0 and %s order by `user`",
...
#ifdef __METASTACK_OPT_USER_DEACTIVATE
	if (is_deactivate) 
		rc = deactivate_common(kingbase_conn, DBD_DEACTIVATE_ACCOUNT_COORDS,
			       ...
			       slurmdb_user_cond_t *user_cond)
{
	...
	if (user_cond->with_deleted == SLURMDB_QUERY_WITH_DELETED)
		xstrfmtcat(extra, "where (deleted=0 or deleted=1 or deleted=%d)", SLURMDB_USER_DEACTIVATED);
	else if (user_cond->with_deleted == SLURMDB_QUERY_WITH_DEACTIVATED)
		xstrfmtcat(extra, "where (deleted=0 or deleted=%d)", SLURMDB_USER_DEACTIVATED);
