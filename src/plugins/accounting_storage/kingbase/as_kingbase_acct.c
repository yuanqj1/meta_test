...
static int _foreach_flag_coord_user(void *x, void *arg)
{
	...
	if (assoc_ptr->user) {
		slurmdb_user_rec_t *user_rec = as_kingbase_user_add_coord_update(
			flag_coord_acct->kingbase_conn,
...
static int _foreach_flag_coord_acct(void *x, void *arg)
{
	...
	if (assoc_mgr_fill_in_assoc(flag_coord_acct->kingbase_conn,
				     &assoc_req,
	...
	if (((assoc_ptr->flags & ASSOC_FLAG_USER_COORD) &&
		...
		if (addto_update_list(flag_coord_acct->kingbase_conn->update_list,
...
static int _foreach_flag_coord_cluster(void *x, void *arg)
{
	...
	if (flag_coord_acct->query) {
		...
		DB_DEBUG(DB_ASSOC, flag_coord_acct->kingbase_conn->conn,
			 "query\n%s",
			 flag_coord_acct->query);
		if ((rc = kingbase_db_query(flag_coord_acct->kingbase_conn,
					 flag_coord_acct->query)) !=
		    SLURM_SUCCESS) {
...
static void _handle_flag_coord(flag_coord_acct_t *flag_coord_acct)
{
	...
	(void) list_for_each_ro(as_kingbase_cluster_list,
...
static void _setup_acct_cond_limits(slurmdb_account_cond_t *acct_cond,
					char **extra, char **at, bool *list_all)
#else
static void _setup_acct_cond_limits(slurmdb_account_cond_t *acct_cond,
				    char **extra, char **at)
#endif
				     ...
				     slurmdb_account_rec_t *acct)
{
	...
	add_acct_cond.kingbase_conn = kingbase_conn;
	...
	if (add_acct_cond.insert_query) {
		...
		rc = kingbase_db_query(kingbase_conn, add_acct_cond.insert_query);
		...
		xstrcatat(add_acct_cond.txn_query,
			  &add_acct_cond.txn_query_pos,
			  ";");
		rc = kingbase_db_query(kingbase_conn, add_acct_cond.txn_query);
		if (rc != SLURM_SUCCESS) {
			error("Couldn't add txn");
			rc = SLURM_SUCCESS;
		}
...
empty:
	...
	for(i = 0 ; i < row ; i++){
		...
		if (acct_cond && (acct_cond->flags & SLURMDB_ACCT_FLAG_WCOORD))
			acct->coordinators =
				assoc_mgr_acct_coords(kingbase_conn, acct->name);
