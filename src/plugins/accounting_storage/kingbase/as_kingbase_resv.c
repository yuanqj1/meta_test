/*****************************************************************************\
 *  as_kingbase_resv.c - functions dealing with reservations.
 *****************************************************************************
...
static void _get_usage_for_resv(kingbase_conn_t *kingbase_conn, uid_t uid,
				...
				char *resv_id)
{
	...
	job_list = as_kingbase_jobacct_process_get_jobs(
			kingbase_conn, uid, &job_cond);
...
extern int as_kingbase_add_resv(kingbase_conn_t *kingbase_conn,
			     slurmdb_reservation_rec_t *resv)
{
	...
	DB_DEBUG(DB_RESV, kingbase_conn->conn, "query\n%s", query);
...
extern int as_kingbase_modify_resv(kingbase_conn_t *kingbase_conn,
				slurmdb_reservation_rec_t *resv)
{
	...
	do {
		...
		if (j == cnt) {
			...
			rc = as_kingbase_add_resv(kingbase_conn, resv);
			goto end_it;
		}
	} while (slurm_atoul(KCIResultGetColumnValue(result, j, RESV_DELETED)));

	start = slurm_atoul(KCIResultGetColumnValue(result, j, RESV_START));
    ...
    for(int k = j + 1; k < cnt; k++) {
		if (resv->time_start != slurm_atoul(KCIResultGetColumnValue(result, k, RESV_START)))
			continue;
	...
	if (!resv->name
	    && KCIResultGetColumnValue(result, j, RESV_NAME) && KCIResultGetColumnValue(result, j, RESV_NAME)[0])
	...
	if (!resv->time_end)
		resv->time_end = slurm_atoul(KCIResultGetColumnValue(result, j, RESV_END));
...
extern int as_kingbase_remove_resv(kingbase_conn_t *kingbase_conn,
				slurmdb_reservation_rec_t *resv)
{
	int rc = SLURM_SUCCESS;
	char *query = NULL;
...
extern List as_kingbase_get_resvs(kingbase_conn_t *kingbase_conn, uid_t uid,
	...
	if (resv_cond->nodes) {
		...
		if (!list_count(resv_cond->cluster_list))
			list_append(resv_cond->cluster_list,
				    xstrdup(kingbase_conn->cluster_name));
...
empty:
	...
	for(int j = 0; j < cnt; j++) {
		slurmdb_reservation_rec_t *resv;
		int start = slurm_atoul(KCIResultGetColumnValue(result, j, RESV_REQ_START));

		if (!good_nodes_from_inx(local_cluster_list, &curr_cluster,
					 KCIResultGetColumnValue(result, j, RESV_REQ_NODE_INX), start))
			continue;

		resv = xmalloc(sizeof(slurmdb_reservation_rec_t));
		list_append(resv_list, resv);
		resv->id = slurm_atoul(KCIResultGetColumnValue(result, j, RESV_REQ_ID));
		resv->name = xstrdup(KCIResultGetColumnValue(result, j, RESV_REQ_NAME));
		resv->node_inx = xstrdup(KCIResultGetColumnValue(result, j, RESV_REQ_NODE_INX));
		resv->cluster = xstrdup(KCIResultGetColumnValue(result, j, RESV_REQ_COUNT));
		resv->assocs = xstrdup(KCIResultGetColumnValue(result, j, RESV_REQ_ASSOCS));
		resv->nodes = xstrdup(KCIResultGetColumnValue(result, j, RESV_REQ_NODES));
		resv->time_start = start;
		resv->time_end = slurm_atoul(KCIResultGetColumnValue(result, j, RESV_REQ_END));
