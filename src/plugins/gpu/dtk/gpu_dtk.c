...
#define DTK_REQ_VERSION_USAGE 2
#endif
/*
 ...
 */
const char plugin_name[] = "GPU DTK plugin";
const char	plugin_type[]		= "gpu/dtk";
...
static void _dtk_get_version(char *version, unsigned int len);
...
extern int init(void)
{
	...
	if (running_in_slurmstepd()) {
		...
		_dtk_get_version(version, RSMI_STRING_BUFFER_SIZE);
		/*
 ...
 */
static bool _dtk_get_mem_freqs(uint32_t dv_ind,
 ...
 * Return true if successful, false if not.
 */
static bool _dtk_get_gfx_freqs(uint32_t dv_ind,
 ...
 */
static void _dtk_print_freqs(uint32_t dv_ind, log_level_t l)
{
	...
	if (!_dtk_get_mem_freqs(dv_ind, &size, mem_freqs))
		return;
	...
	if (!_dtk_get_gfx_freqs(dv_ind, &size, gfx_freqs))
		return;
 ...
 */
static void _dtk_get_nearest_freqs(uint32_t dv_ind,
				    ...
				    uint64_t *gfx_bitmask)
{
	...
	if (!_dtk_get_mem_freqs(dv_ind, &mem_freqs_size, mem_freqs))
		return;
	...
	if (!_dtk_get_gfx_freqs(dv_ind, &gfx_freqs_size, gfx_freqs))
		return;
 ...
 */
static bool _dtk_set_freqs(uint32_t dv_ind, uint64_t mem_bitmask,
			    uint64_t gfx_bitmask)
{
	const char *status_string;
	rsmi_status_t rsmi_rc;
 ...
 * Returns true if successful, false if not
 */
static bool _dtk_reset_freqs(uint32_t dv_ind)
{
	const char *status_string;
	rsmi_status_t rsmi_rc;
 ...
 */
static unsigned int _dtk_get_freq(uint32_t dv_ind, rsmi_clk_type_t type)
{
	const char *status_string;
	rsmi_status_t rsmi_rc;
	rsmi_frequencies_t rsmi_freqs;
...
static unsigned int _dtk_get_gfx_freq(uint32_t dv_ind)
{
	return _dtk_get_freq(dv_ind, RSMI_CLK_TYPE_SYS);
}

static unsigned int _dtk_get_mem_freq(uint32_t dv_ind)
{
	return _dtk_get_freq(dv_ind, RSMI_CLK_TYPE_MEM);
}
...
static void _set_freq(bitstr_t *gpus, char *gpu_freq)
{
	...
	for (i = 0; i < gpu_len; i++) {
		...
		_dtk_get_nearest_freqs(i, &mem_freq, &mem_bitmask,
		...
		debug2("Graphics frequency before set: %u",
		       _dtk_get_gfx_freq(i));
		freq_set = _dtk_set_freqs(i, mem_bitmask, gpu_bitmask);
 ...
 */
static void _dtk_get_driver(char *driver, unsigned int len)
{
...
static void _dtk_get_version(char *version, unsigned int len)
{
	...
	if (rsmi_rc != RSMI_STATUS_SUCCESS) {
		...
		if (rsmi_version.major < DTK_REQ_VERSION_USAGE) {
			get_usage = false;
			error("%s: GPU usage accounting disabled. dtk version >= 2.0.0 required.",
			      __func__);
		}
	}
}
 ...
 */
static void _dtk_get_device_name(uint32_t dv_ind, char *device_name,
				  unsigned int size)
{
	const char *status_string;
 ...
 * size		(OUT) Size of name
 */
static void _dtk_get_device_brand(uint32_t dv_ind, char *device_brand,
				   unsigned int size)
{
	const char *status_string;
 ...
 */
static void _dtk_get_device_minor_number(uint32_t dv_ind,
					  unsigned int *minor)
{
	const char *status_string;
 ...
 */
static void _dtk_get_device_pci_info(uint32_t dv_ind, rsmiPciInfo_t *pci)
{
	const char *status_string;
 ...
 */
// static void _dtk_get_device_unique_id(uint32_t dv_ind, uint64_t *id)
// {
// 	const char *status_string;
...
static bitstr_t *_dtk_get_device_cpu_mask(uint32_t dv_ind)
{
 ...
 */
static List _get_system_gpu_list_dtk(node_config_load_t *node_config)
{
	unsigned int i;
	...
	for (i = 0; i < device_count; ++i) {
		...
		gres_slurmd_conf_t gres_slurmd_conf = {
			...
			.cpus_bitmap = _dtk_get_device_cpu_mask(i),
			.name = "dcu",
		};
		...
		_dtk_get_device_name(i, device_name, RSMI_STRING_BUFFER_SIZE);
		_dtk_get_device_brand(i, device_brand,
...
// 			dcu += 1;
// 		} else if (xstrcasecmp(device_brand, "BW3000") == 0) {
// 			dcu += 1;
// 		} else {
// 			_dtk_get_device_unique_id(i, &uuid);
// 		}
// #endif
		_dtk_get_device_minor_number(i, &minor_number);
		pci_info.bdfid = 0;
		_dtk_get_device_pci_info(i, &pci_info);
		...
		_dtk_print_freqs(i, LOG_LEVEL_DEBUG2);
...
{
	List gres_list_system = _get_system_gpu_list_dtk(node_config);
...
extern int gpu_p_energy_read(uint32_t dv_ind, gpu_status_t *gpu)
{
	...
	END_TIMER;
	debug3("rsmi_dev_power_ave_get took %ld microseconds", DELTA_TIMER);
	if (rsmi_rc != RSMI_STATUS_SUCCESS) {
		rsmi_rc = rsmi_status_string(rsmi_rc, &status_string);
...
extern int gpu_p_usage_read(pid_t pid, acct_gather_data_t *data)
{
	...
	START_TIMER;
	gpu_p_get_device_count(&device_count);
	//debug3("Device count: %d", device_count);
	rc = rsmi_compute_process_info_by_pid_get(pid, &proc);
	END_TIMER;
	debug3("rsmi_compute_process_info_by_pid_get took %ld microseconds", DELTA_TIMER);
	if (rc == RSMI_STATUS_NOT_FOUND) {
		...
		/* 计算利用率= 总卡数 * 归一化后的数据 */
		if (track_gpuutil) {
			data[gpuutil_pos].size_read = (proc.cu_occupancy) * device_count;
			log_flag(JAG, "pid %d has GPUUtil=%lu", pid, data[gpuutil_pos].size_read);
		}
			
		if (track_gpumem) {
			data[gpumem_pos].size_read = proc.vram_usage;
			log_flag(JAG, "pid %d has MemMB=%lu", pid, data[gpumem_pos].size_read /1048576);
		}
			
		return SLURM_SUCCESS;
	} else {
		(void) rsmi_status_string(rc, &status_string);
		error("DTK: Failed to get usage(%d): %s ; change to other way", rc, status_string);
		// rc = rsmi_compute_process_info_by_pid_get_v2(pid, &proc_v2);
		// if (rc == RSMI_STATUS_SUCCESS ) {
		// 	for (int i = 0; i < proc_v2.usedGpus; i++) {
		// 		debug4("GPU %d usage: %.2f%%\n", proc_v2.gpuIndex[i], proc_v2.gpuUsageRate[i]);
		// 		if (track_gpuutil)
		// 			data[gpuutil_pos].size_read +=  proc_v2.gpuUsageRate[i];
		// 		if (track_gpumem)
		// 			data[gpumem_pos].size_read += proc_v2.vramUsageSize/ 1048576;
		// 	}
