...
extern int             persist_acct_update_count;
...
extern void record_cache_rpc_stats(uint16_t msg_type, slurm_msg_t *msg, long delta)
{
...
static void _fill_ctld_conf(slurm_conf_t *conf_ptr)
{
...
#ifdef __METASTACK_OPT_CACHE_QUERY
	conf_ptr->cachedup_interval   = conf->cachedup_interval;
	...
	conf_ptr->skip_save_sacct_again= conf->skip_save_sacct_again;
#endif
    ...
    conf_ptr->suspend_idle_def	  = conf->suspend_idle_def;
#endif
...
#ifdef __METASTACK_OPT_CACHE_QUERY
	if(!error_code && job_cachedup_realtime != 0){
	...
	while ((job_ptr = list_next(iter))) {
		...
		if(!error_code){
			if(job_cachedup_realtime == 1 && job_ptr && find_job_record(job_ptr->job_id)){
				cache_job_ptr = _meta_add_cache_job(job_ptr);
			...
			}else if(job_cachedup_realtime == 2 && job_ptr && find_job_record(job_ptr->job_id)){
...
static void _slurm_rpc_dump_cache_jobs(slurm_msg_t * msg)
{
	...
	slurmctld_lock_t job_read_lock = {
		READ_LOCK, NO_LOCK, NO_LOCK, NO_LOCK, READ_LOCK };
	slurmctld_lock_t job_cache_read_lock = {
		NO_LOCK, READ_LOCK, NO_LOCK, READ_LOCK, NO_LOCK };
	START_TIMER;
	if (!(msg->flags & CTLD_QUEUE_PROCESSING)){
		lock_cache_query(job_cache_read_lock);
		lock_slurmctld(job_read_lock);
	}
	...
	if ((job_info_request_msg->last_update - 1) >= last_job_update) {
		if (!(msg->flags & CTLD_QUEUE_PROCESSING)){
			unlock_slurmctld(job_read_lock);
			unlock_cache_query(job_cache_read_lock);
		}
		debug3("_slurm_rpc_dump_jobs, no change");
		slurm_send_rc_msg(msg, SLURM_NO_CHANGE_IN_DATA);
	} else {
		if (job_info_request_msg->job_ids) {
			debug2("%s CACHE_QUERY call pack_spec_cache_jobs", __func__);
			buffer = pack_spec_cache_jobs(job_info_request_msg->job_ids,
						job_info_request_msg->show_flags,
						msg->auth_uid, NO_VAL,
						msg->protocol_version);     
		} else {
			debug2("%s CACHE_QUERY call pack_all_cache_jobs", __func__);
			buffer = pack_all_cache_jobs(job_info_request_msg->show_flags,
					       msg->auth_uid, NO_VAL,
					       msg->protocol_version);
		}
		if (!(msg->flags & CTLD_QUEUE_PROCESSING)){
...
/* _slurm_rpc_dump_cache_jobs_user - process RPC for job state information */
static void _slurm_rpc_dump_cache_jobs_user(slurm_msg_t * msg)
{
	DEF_TIMERS;
	buf_t *buffer = NULL;
	slurm_msg_t response_msg;
	...
	debug2("%s CACHE_QUERY call pack_all_cache_jobs", __func__);
	buffer = pack_all_cache_jobs(job_info_request_msg->show_flags, msg->auth_uid,
...
/* _slurm_rpc_dump_cache_job_single - process RPC for one job's state information */
static void _slurm_rpc_dump_cache_job_single(slurm_msg_t * msg)
{
	DEF_TIMERS;
	buf_t *buffer = NULL;
	slurm_msg_t response_msg;
	...
	debug2("%s CACHE_QUERY call pack_one_cache_job", __func__);
	buffer = pack_one_cache_job(job_id_msg->job_id, job_id_msg->show_flags,
...
/* _slurm_rpc_dump_cache_partitions - process RPC for partition state information */
static void _slurm_rpc_dump_cache_partitions(slurm_msg_t * msg)
{
	DEF_TIMERS;
	buf_t *buffer = NULL;
	slurm_msg_t response_msg;
	...
	slurmctld_lock_t part_cache_read_lock = {
	...
	if (!(msg->flags & CTLD_QUEUE_PROCESSING)){
		lock_cache_query(part_cache_read_lock);
	...
	if ((part_req_msg->last_update - 1) >= last_part_update) {
		if (!(msg->flags & CTLD_QUEUE_PROCESSING)){
			...
			unlock_cache_query(part_cache_read_lock);
		}
		debug2("_slurm_rpc_dump_partitions, no change");
		slurm_send_rc_msg(msg, SLURM_NO_CHANGE_IN_DATA);
	} else {
		debug2("%s CACHE_QUERY call pack_all_cache_part", __func__);
		buffer = pack_all_cache_part(part_req_msg->show_flags, msg->auth_uid,
					msg->protocol_version);
		if (!(msg->flags & CTLD_QUEUE_PROCESSING)){
			unlock_slurmctld(part_read_lock);
			unlock_cache_query(part_cache_read_lock);
...
/* _slurm_rpc_dump_cache_nodes - dump RPC for node state information */
static void _slurm_rpc_dump_cache_nodes(slurm_msg_t * msg)
{
	DEF_TIMERS;
	buf_t *buffer;
	slurm_msg_t response_msg;
	...
	slurmctld_lock_t conf_read_lock = {
		READ_LOCK, NO_LOCK, NO_LOCK, NO_LOCK, NO_LOCK };
	slurmctld_lock_t node_cache_read_lock = {
	...
	if ((node_req_msg->last_update - 1) >= last_node_update) {
		if (!(msg->flags & CTLD_QUEUE_PROCESSING)){
			...
			unlock_cache_query(node_cache_read_lock);
		}
		debug3("_slurm_rpc_dump_nodes, no change");
		slurm_send_rc_msg(msg, SLURM_NO_CHANGE_IN_DATA);
	} else {
		debug2("%s CACHE_QUERY call pack_all_cache_node", __func__);
		buffer = pack_all_cache_node(node_req_msg->show_flags,
					msg->auth_uid, msg->protocol_version);
		if (!(msg->flags & CTLD_QUEUE_PROCESSING)){
			unlock_slurmctld(conf_read_lock);
			unlock_cache_query(node_cache_read_lock);
...
/* _slurm_rpc_dump_cache_node_single - done RPC state information for one node */
static void _slurm_rpc_dump_cache_node_single(slurm_msg_t * msg)
{
	DEF_TIMERS;
	buf_t *buffer = NULL;
	slurm_msg_t response_msg;
...
#endif
	debug2("%s CACHE_QUERY call pack_one_cache_node", __func__);
	buffer = pack_one_cache_node(node_req_msg->show_flags, msg->auth_uid,
...
static void _slurm_rpc_dump_conf_watch_dog(slurm_msg_t * msg)
{
	...
	if ((last_time_msg->last_update - 1) >= last_watch_dog_update) {
		...
		buffer = pack_all_watch_dog(msg->auth_uid, msg->protocol_version);
		unlock_slurmctld(watch_dog_read_lock);
		...
		response_init(&response_msg, msg, RESPONSE_BUILD_WATCH_DOG_INFO,
			      buffer);
		/* send message */
		slurm_send_node_msg(msg->conn_fd, &response_msg);
		//xfree(dump);
...
{  
	...
	if (error_code) {  
		info("%s AppName=%s Version=%s: %s",  
		     ...
		     app_desc_ptr->versions ? app_desc_ptr->versions : "?",  
...
static void _slurm_rpc_job_step_create(slurm_msg_t *msg)
{
	...
	/*Using srun to carry the watch dog again in sbatch or sbatch requires verification here*/
	watch_dog_record_t *watch_dog_ptr = NULL;
	if(req_step_msg->watch_dog) {
		
		int error_code = SLURM_SUCCESS;
		error_code =  _get_job_watch_dogs_and_check(req_step_msg->watch_dog, &watch_dog_ptr, NULL);
		error("the jobid %d has an invalid watchdog specified watch_dog_ptr %s",req_step_msg->step_id.job_id,
							req_step_msg->watch_dog);
		if(error_code == ESLURMD_INVALID_WATCH_DOG)
			error("the jobid %d has an invalid watchdog specified",req_step_msg->step_id.job_id);
	}
	...
	if (!step_create_from_msg(msg,
					...
					((!(msg->flags & CTLD_QUEUE_PROCESSING)) ?
					_step_create_job_fail_lock : NULL), watch_dog_ptr)) {
#endif
	...
	if ((slurm_conf.private_data & PRIVATE_DATA_JOBS) &&
		...
		if (slurm_mcs_get_privatedata()) {
			if (mcs_g_check_mcs_label_user(&args->user_rec,
						  job_ptr->mcs_label))
				return 0;
		} else if (!assoc_mgr_is_user_acct_coord_user_rec(
							 &args->user_rec,
							 job_ptr->account)) {
			return 0;
		}
...
static void _slurm_rpc_job_step_get_info(slurm_msg_t *msg)
{
	...
	if ((request->last_update - 1) >= last_job_update) {
		...
		args.user_rec.uid =  msg->auth_uid;
		...
		*/
		...
		/**
		 * args.user_rec.assoc_list may have been changed 
		 * in func: assoc_mgr_fill_in_user
		 */
		args.user_rec.assoc_list = NULL;
		if (!skip_visible_parts) {
			args.user_rec.assoc_list = fill_assoc_list(args.user_rec.uid, 
					true);
			args.visible_parts = build_visible_parts_user(&args.user_rec, 
					true);
		}
...
static void _slurm_rpc_job_alloc_info(slurm_msg_t *msg)
{
	...
	lock_slurmctld(job_read_lock);
#ifdef __METASTACK_OPT_PART_VISIBLE
	assoc_mgr_lock_t assoc_locks = { .assoc = READ_LOCK, .user = READ_LOCK };
#ifdef __METASTACK_ASSOC_HASH
	...
	if (!job_ptr->het_job_list) {
		...
		while ((het_job = list_next(iter))) {
			...
			if (het_job->job_id != job_info_msg->job_id)
				(void) job_alloc_info_ptr_user(&user_rec,
							  het_job);
#else
			if (het_job->job_id != job_info_msg->job_id)
...
static void _slurm_rpc_job_sbcast_cred(slurm_msg_t *msg)
{
	...
	if (job_info_msg->het_job_offset == NO_VAL) {
		...
		list_itr_t *iter;
#ifdef __METASTACK_OPT_PART_VISIBLE
		error_code = job_alloc_info_user(&user_rec, 
...
#ifdef __METASTACK_OPT_PART_VISIBLE
				error_code = job_alloc_info_ptr_user(&user_rec, 
							        het_job_ptr);
#else
...
#ifdef __METASTACK_OPT_PART_VISIBLE
			error_code = job_alloc_info_user(
				&user_rec, job_info_msg->step_id.job_id,
				&job_ptr);
#else
...
static void _slurm_rpc_request_control(slurm_msg_t *msg)
{
	...
	if (slurmctld_config.shutdown_time) {
		...
		/* signal clean-up */
#ifdef __METASTACK_BUG_SPLIT_BRAIN
...
#ifdef __METASTACK_OPT_CACHE_QUERY
	if(!reject_job && job_cachedup_realtime != 0){
		submit_cache_job_list = list_create(NULL);
	}
#endif
	iter = list_iterator_create(submit_job_list);
...
static void _slurm_rpc_update_job(slurm_msg_t *msg)
{
	...
	START_TIMER;
#ifdef __META_PROTOCOL
	if ((msg->protocol_version >= META_2_0_PROTOCOL_VERSION) &&
...
#ifdef __METASTACK_BUG_USERADD_RECONFIG_CTLD_CRASH
		/**fix bug 104294
		 * Ensure all PERSIST_TYPE_ACCT_UPDATE persists have sent their msg before reconfig.
		 * And make sure that no child threads are created after shutdown.
		 */
		if (slurmctld_config.shutdown_time) {
			is_destroy_conn = true;
			rc = SLURM_ERROR;
		} else {
			slurm_mutex_lock(&persist_count_lock);
			persist_acct_update_count++;
			slurm_mutex_unlock(&persist_count_lock);
...
#ifdef __METASTACK_BUG_USERADD_RECONFIG_CTLD_CRASH
	if (persist_init->persist_type == PERSIST_TYPE_ACCT_UPDATE && !is_destroy_conn && (persist_acct_update_count > 0)) {
		slurm_mutex_lock(&persist_count_lock);
		persist_acct_update_count--;
		if (persist_acct_update_count == 0) {
			slurm_cond_broadcast(&persist_count_cond);
		}
		slurm_mutex_unlock(&persist_count_lock);
	}
#endif
...
{
	{
		...
		.func = _slurm_rpc_dump_cache_jobs,
	},{
		.msg_type = REQUEST_CACHE_JOB_USER_INFO,
		.func = _slurm_rpc_dump_cache_jobs_user,
	},{
		.msg_type = REQUEST_CACHE_JOB_INFO_SINGLE,
		.func = _slurm_rpc_dump_cache_job_single,	
	},{
		.msg_type = REQUEST_CACHE_PARTITION_INFO,
		.func = _slurm_rpc_dump_cache_partitions,
	},{
		.msg_type = REQUEST_CACHE_NODE_INFO,
		.func = _slurm_rpc_dump_cache_nodes,
	},{
...
void slurmctld_query_req(slurm_msg_t *msg)
{
	...
	uint16_t msg_type;   
	if (msg->msg_type == REQUEST_JOB_INFO){
		msg_type = REQUEST_CACHE_JOB_INFO;
		...
		msg_type = REQUEST_CACHE_PARTITION_INFO;
	}else if(msg->msg_type == REQUEST_NODE_INFO){
		msg_type = REQUEST_CACHE_NODE_INFO;
	}else if(msg->msg_type == REQUEST_NODE_INFO_SINGLE){
		msg_type = REQUEST_CACHE_NODE_INFO_SINGLE;
	}else{
	...
	*/
	START_TIMER;
	if (slurm_conf.debug_flags & DEBUG_FLAG_PROTOCOL) {
		const char *p = rpc_num2string(msg_type);
		if (msg->conn) {
			info("%s: received opcode %s from persist conn on (%s)%s uid %u",
				__func__, p, msg->conn->cluster_name,
				msg->conn->rem_host, msg->auth_uid);
	...
	debug2("Processing RPC: %s from UID=%u",
		rpc_num2string(msg_type), msg->auth_uid);
	
	for (int i = 0; slurmctld_rpcs[i].msg_type; i++) {
		if (slurmctld_rpcs[i].msg_type != msg_type)
			continue;
	...
	if (this_rpc) {
		(*(this_rpc->func))(msg);
		END_TIMER;
		record_cache_rpc_stats(msg_type, msg, DELTA_TIMER);
	} else {
