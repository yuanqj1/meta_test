...
typedef struct slurmctld_config {
	...
	pthread_rwlock_t thread_count_rwlock;
#endif		
...
extern list_t *queue_license_list; /* list of queue_license_list  */
#endif
...
/*****************************************************************************\
 *  Global para_sched variables
\*****************************************************************************/
#ifdef __METASTACK_NEW_PART_PARA_SCHED
/* Global variables */
extern int part_count;     /* The number of all partitions */
...
extern part_record_t ***para_sched_part_ptr;   /* A collection of part_ptr for all parts in each resource area */
/* for nodes */
extern bitstr_t **original_part_node_bitmap;    /* A collection of bitmaps for all nodes in each partition */
extern bitstr_t **para_sched_node_bitmap;       /* A collection of bitmaps for all nodes in each resource area */
...
/* slurm_get_resource_node_detail - get job info about resource(CPU, Mem, GRES) in node
 * IN job_ptr - pointer to job
 * IN in_s - Internal separator in single result string
 * IN out_s - External separator between result strings
 * OUT - result strings of a job info about resource(CPU, Mem, GRES) in node
 */
extern char* slurm_get_resource_node_detail(struct job_record* job_ptr, char in_s, char out_s);
#endif
...
typedef struct {
	...
	time_t start_time;
	time_t end_time;
	uint32_t time_min;		/* minimum time_limit minutes or INFINITE,
...
typedef struct {
	...
	char *deny_accounts;
	list_t *deny_accts_list;
	list_t *allow_accts_list;
	char *allow_groups;
	char *qos_char;
	char *allow_qos;
	char *deny_qos;
...
#ifdef __METASTACK_OPT_PART_VISIBLE
/**
 * find all assoc records related with (normal user) uid
 * return assoc_list
 */
extern List fill_assoc_list(uid_t uid, bool locked);

/**
 * build_visible_parts_user - find part that user could visible/submit
 * based on assoc's part, part's allowgroup/allowaccount, part's hidden
 * return array with pointers to partitions visible
 */
