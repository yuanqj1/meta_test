...
static int _sacct_parse_extend (char* format_long) 
{
	...
	if (format_long == NULL) {
		error("Format option lacks specification of sacct");
		return SLURM_ERROR;
	}
	...
	while (token) {
		...
		if (!xstrcasecmp(token, "all")) {
			for (i = 0; fields[i].name; i++) {
				if (field_size)
					fields[i].len = field_size;
				fields[i].right_format = right_format;
			}
			token = strtok_r(NULL, ",", &str_tmp);
			continue;
		}
		for (i = 0; fields[i].name; i++) {
			if (!xstrncasecmp(fields[i].name, token, command_len))
				goto foundfield;
		}

		if (!xstrcasecmp("AllocGRES", token)) {
			fatal("AllocGRES has been removed, please use AllocTRES of SACCT_EXTEND");
		}

		if (!xstrcasecmp("ReqGRES", token)) {
			fatal("ReqGRES has been removed, please use ReqTRES of SACCT_EXTEND");
		}

		error("Invalid field requested in env: \"%s\"", token);
		exit(1);
foundfield:
		if (field_size)
			fields[i].len = field_size;
...
#ifdef __METASTACK_OPT_PRINT_COMMAND
	if ((env_sacct = getenv("SACCT_EXTEND"))) {
		char *format = xstrdup(env_sacct);
		_sacct_parse_extend(format);
		if (format)
			xfree(format);
	}
#endif
