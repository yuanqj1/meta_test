...
#include <sys/stat.h>		/* Obtain the information of the file located at the specified path */
#endif
#define KB_ADJ 1024
...
#ifdef __METASTACK_NEW_LOAD_ABNORMAL
/* Function for collecting performance metrics at the task level
 * collect - collect resource data for each task
 * data - capture resource usage information for the job step running on this node.
 */
...
static void *apptype_recognition(void *args){
	if (args == NULL) {
		debug3("Since the parameter passed to the acctg_apptype thread was NULL, the thread exited. However, this situation should not occur.");
		return NULL;
	}
	...
	time_t start_time = time(NULL); // Record the start time of the thread
	...
	/* Mark thread survival */
	slurm_mutex_lock(&apptype_thread_mutex);
	apptype_thread_running = true;
	slurm_mutex_unlock(&apptype_thread_mutex);
	...
	*/
	...
	while (_init_run_test() && !_jobacct_shutdown_test() &&
		...
		*/
		int limit = MIN(collect->app_rec_cnt, JOBACCTINFO_START_END_ARRAY_SIZE);
		...
		if (have_recogn || have_recogn_weak) {
			...
			if (profile & ACCT_GATHER_PROFILE_APPTYPE) {
				...
				if (time(NULL) - start_time >= JOBACCTGATHER_APPTYPE_THREAD_DURATION) {
					/* send job apptype data to influxdb */
					write_data->have_recogn = true;
					_poll_data(1, NULL, write_data);
				} else {
		...
		/*
			Check if it exceeds JOBACCTGATHER_APPTYPE_THREAD_DURATION seconds 
			
			Place this judgment at the end of the while loop to ensure that the loop is executed at least 
			once, so that the result of identifying the application name can be sent to influxdb (if the 
			influxdb plugin is enabled) before the thread exits.
		*/
		if (time(NULL) - start_time >= JOBACCTGATHER_APPTYPE_THREAD_DURATION) {
			debug3("apptype_recognition: %d seconds timeout reached, exiting", JOBACCTGATHER_APPTYPE_THREAD_DURATION);
			break;
		}
	...
	/*
		Print the recognition results to the log of the computing node. Although collecting the recognition 
		results of the application name from all the computing node logs is a cumbersome task(We recommend 
		enabling the InfluxDB plugin instead.), we still wish to retain these data.
	*/
	if (apptype_step)
...
static char **_build_watch_dog_env(acct_gather_rank_t *watch_dog)
{
	...
	if (watch_dog->app_name) {
		setenvf(&my_env, "SLURM_JOB_APP_SOURCE", "%s",
			app_source_to_str(watch_dog->app_source));
	}
#endif
	...
	while (_init_run_test() && !_jobacct_shutdown_test() &&
		...
		if(ret == 0) {
			...
			debug3("[job %u] attempting to run  [%s]",
					run_script_msg.job_id, run_script_msg.script_path);
...
static void data_summation(write_t *write_data, step_gather_msg_t* msg, acct_gather_rank_t *job_info, int type)
{
			...
			if((step_gather.children_gather <= 0) && (step_gather.parent_rank_gather >= 0) ) {
				...
				if((step_gather.wait_child_count == step_gather.children_gather) || (diff >= time_delay)) {
					...
					if(step_gather.wait_child_count == step_gather.children_gather) {
						data_summation(NULL, &msg, NULL, SECOND_TREE);
				...
				if((step_gather.children_gather == -1 ) || (step_gather.wait_child_count == step_gather.children_gather) || (diff >= time_delay)) {
					...
					if(step_gather.wait_child_count == step_gather.children_gather) {
						data_summation(NULL, &msg, NULL, THREE_TREE);
					} else if(diff >= time_delay) {
						data_summation(NULL, &msg, NULL, THREE_TREE);
						msg.load_flag  = msg.load_flag | JNODE_STAT;
					}
					data_summation(write_data, &msg, job_info, PARENT_SUM);
                    head = true;
					update = false;
...
#endif
	if((job_message->timer) > 0 && (job_message->frequency > 0))
...
extern int apptype_properties_conf_init(void)
{
	...
	/* 
		slurmstepd and slurmctld does not need to read apptype.propertes, is derived from slurmd
	*/
	...
	*/
	...
	if ((conf_path == NULL) || (stat(conf_path, &buf) == -1)) {
		...
		/* 
			Build the hash table from apptype.properties, passing PARSE_FLAGS_IGNORE_NEW and changing error to debug 
			so as not to affect the ctld service 
		*/
		if (s_p_parse_file(tbl, NULL, conf_path, PARSE_FLAGS_IGNORE_NEW, NULL) ==
		    SLURM_ERROR) {
			/*
				We hope that the newly added application name recognition function will not interfere with the normal 
				operation of the services. Therefore, here we only need to return the error message instead of directly 
				using "fatal".
			*/
			error("Could not open/read/parse apptype.properties file "
...
extern int jobacct_gather_apptypepoll(uint16_t frequency, acct_gather_rank_t *jobinfo) 
{
	...
	*/
	args = (apptype_recogn_thread_args_t*)xmalloc(sizeof(apptype_recogn_thread_args_t));
					...
					uint16_t protocol_type, buf_t *buffer)
{
	...
	if (rpc_version >= META_3_2_PROTOCOL_VERSION) {
		...
		// Skip the packaging step during build
	} else if(rpc_version >= SLURM_ONE_BACK_PROTOCOL_VERSION) {
		// Skip the packaging step during build
	} else if(rpc_version >= SLURM_TWO_BACK_PROTOCOL_VERSION)	{
		// Skip the packaging step during build
	} else if(rpc_version >= META_2_1_PROTOCOL_VERSION){
		if ((tmp_64 = jobacct->flag) < 0)
			tmp_64 = 0;
		pack64(tmp_64, buffer);
			     ...
			     uint16_t protocol_type, buf_t *buffer)
{
	...
	if (rpc_version >= META_3_2_PROTOCOL_VERSION) {
		...
		/**
		 * offical version: 24.05, 23.02, 23.11
		 */
		pack64(jobacct->user_cpu_sec, buffer);
		pack32((uint32_t)jobacct->user_cpu_usec, buffer);
		pack64(jobacct->sys_cpu_sec, buffer);
