...
#ifdef __METASTACK_BUG_SLURMSTEPD_READ_ACCT_GATHER_CONF
	/* slurmstepd is no need to read acct_gather.conf, only receives from slurmd */
...
extern int acct_gather_parse_freq(int type, char *freq)
{
	...
	case PROFILE_APPTYPE:
		if ((sub_str = xstrcasestr(freq, "apptype="))){
			freq_int = _get_int(sub_str + 8);
			freq_int = MIN(freq_int, JOBACCTGATHER_APPTYPE_THREAD_DURATION);
		}
		break;
#endif
	default:
										...
										launch_tasks_request_msg_t *msg)
{
	...
	if((msg->period > 0) && msg->watch_dog && msg->watch_dog_script) {
		...
		if(msg->enable_all_nodes == true) {
			...
			if((msg->style_step  == JOB_SUBMIT_SRUN) && (stepd == 0))
				watch_dog_start = true;
			/*use salloc submit job*/
			if((msg->style_step  == JOB_SUBMIT_ALLOC) && (stepd == SLURM_EXTERN_CONT))
				watch_dog_start = true;
			//*head_node = true;
		}
	}
