...
static uint32_t dcu_plugin_id = NO_VAL;
#endif
#ifdef __METASTACK_NEW_GRES_NPU
static uint32_t npu_plugin_id = NO_VAL;
#endif
...
#ifdef __METASTACK_NEW_GRES_DCU
		} else if (!xstrcmp(one_name, "dcu")) {	
			dcu_plugin_id = gres_build_id("dcu");
#endif
#ifdef __METASTACK_NEW_GRES_NPU
		} else if (!xstrcmp(one_name, "npu")) {	
			npu_plugin_id = gres_build_id("npu");
#endif
		}
		if (!skip_name) {
			      ...
			      const char *line, char **leftover)
{
	...
	if (slurm_conf.slurmctld_load_gres && running_in_slurmctld() && slurmctld_load_gres_flag) {
...
extern s_p_hashtbl_t *gres_parse_config_file(char *gres_conf_file, parsed_line_t *parsed_lines, int max_parsed_lines, int *num_parsed_lines) {
	...
	if (stat(gres_conf_file, &config_stat) < 0) {
		...
		if (s_p_parse_gres_file(tbl, NULL, gres_conf_file, 0, NULL, parsed_lines, max_parsed_lines, num_parsed_lines) == SLURM_ERROR) {
				   ...
				   void *xcpuinfo_mac_to_abs)
{
	...
	(void) list_delete_all(gres_conf_list, _find_fileless_gres,
			       &gpu_plugin_id);
#ifdef __METASTACK_NEW_GRES_DCU
	if (dcu_plugin_id != NO_VAL) {
		(void) list_delete_all(gres_conf_list, _find_fileless_gres,
					&dcu_plugin_id);
	}
#endif
#ifdef __METASTACK_NEW_GRES_NPU
	if (npu_plugin_id != NO_VAL) {
		(void) list_delete_all(gres_conf_list, _find_fileless_gres,
					&npu_plugin_id);
	}
#endif
				   ...
				   void *xcpuinfo_mac_to_abs, parsed_line_t *parsed_lines, int max_lines)
{
	...
	if (stat(gres_conf_file, &config_stat) < 0) {
		...
		gres_cpu_cnt = cpu_cnt;
		tbl = s_p_hashtbl_create(_gres_conf_options);
		if (s_p_parse_file_gres(tbl, gres_conf_file, 0, parsed_lines, max_lines, gres_node_name) ==
		    SLURM_ERROR){
	...
	if (gres_js->gres_per_task) {
		if(gres_js->gres_per_job) {
			...
			gres_js->gres_per_job = *gres_js_val->num_tasks *
				gres_js->gres_per_task;
#if defined(__METASTACK_NEW_GRES_DCU) && defined(__METASTACK_NEW_GRES_NPU)
		} else if (!xstrcmp(gres_state_job->gres_name, "gpu") || !xstrcmp(gres_state_job->gres_name, "dcu") || !xstrcmp(gres_state_job->gres_name, "npu") ) {
#endif
...
#ifdef __METASTACK_NEW_GRES_DCU
	} else if (requested_dcu && list_count(*gres_js_val->gres_list)) {
		/* Set num_tasks = dcus * ntasks/dcu */
		uint64_t dcus = _get_job_gres_list_cnt(
			*gres_js_val->gres_list, "dcu", NULL);
		if (dcus != NO_VAL64)
			*gres_js_val->num_tasks = 
			    dcus * (*gres_js_val->ntasks_per_tres);
		else {
...
#endif
#ifdef __METASTACK_NEW_GRES_NPU
	} else if (requested_npu && list_count(*gres_js_val->gres_list)) {
		/* Set num_tasks = npus * ntasks/npu */
		uint64_t npus = _get_job_gres_list_cnt(
			*gres_js_val->gres_list, "npu", NULL);
		if (npus != NO_VAL64)
			*gres_js_val->num_tasks = 
			    npus * (*gres_js_val->ntasks_per_tres);
		else {
...
#if defined(__METASTACK_NEW_GRES_DCU) && defined(__METASTACK_NEW_GRES_NPU)
	if (cpus_per_tres && (!requested_gpu) && (!requested_dcu) && (!requested_npu)) {
#elif defined(__METASTACK_NEW_GRES_DCU)
	if (cpus_per_tres && (!requested_gpu) && (!requested_dcu)) {
#elif defined(__METASTACK_NEW_GRES_NPU)
	if (cpus_per_tres && (!requested_gpu) && (!requested_npu)) {
#else
	if (cpus_per_tres && (!requested_gpu)) {
#endif
#if defined(__METASTACK_NEW_GRES_DCU) || defined(__METASTACK_NEW_GRES_NPU)
		error("Requested cpus_per_tres=%s but did not request any GRES.",
			  cpus_per_tres);
#else
					...
					char * gres_type)
{
	...
	if ((tmp == NO_VAL64) && (*num_tasks != NO_VAL)) {
		...
		char *save_ptr = NULL, *gres = NULL, *in_val = NULL;
		if (strstr(gres_type, "dcu"))
			xstrfmtcat(gres, "gres/dcu:%u", gpus);
		else if (strstr(gres_type, "npu"))
			xstrfmtcat(gres, "gres/npu:%u", gpus);
	...
	if ((ntasks_per_tres != NO_VAL16)) {
		...
		if (xstrstr(tres_per_step, "dcu") || xstrstr(tres_per_node, "dcu") || xstrstr(tres_per_socket, "dcu") || xstrstr(tres_per_task, "dcu")) {
			...
			rc = _handle_ntasks_per_tres_step_withtype(new_step_list,
						  ...
						  gres_type);
			xfree(gres_type);
		} else if (xstrstr(tres_per_step, "npu") || xstrstr(tres_per_node, "npu") || xstrstr(tres_per_socket, "npu") || xstrstr(tres_per_task, "npu")) {
	...
	while ((gres_state_step = (gres_state_t *) list_next(gres_iter))) {
		if (job_node_index == -1)
			...
			new_gres_data = _step_state_dup2(
				gres_state_step->gres_data, job_node_index);
		}
		if (new_gres_list == NULL) {
...
#if defined(__METASTACK_NEW_GRES_DCU) && defined(__METASTACK_NEW_GRES_NPU)
		if (!xstrncasecmp(sep, "map_gpu:", 8) || !xstrncasecmp(sep, "map_dcu:", 8) || !xstrncasecmp(sep, "map_npu:", 8)) { // Old Syntax
			usable_gres = _get_usable_gres_map_or_mask(
				(sep + 8), proc_id, gres_bit_alloc,
				true, get_devices);
		} else if (!xstrncasecmp(sep, "mask_gpu:", 9) || !xstrncasecmp(sep, "mask_dcu:", 9) || !xstrncasecmp(sep, "mask_npu:", 9)) { // Old Syntax
			usable_gres = _get_usable_gres_map_or_mask(
...
extern List gres_node_state_list_copy(List gres_list)
{
	...
	gres_state_t *gres_state_node = NULL, *new_gres = NULL;
	void *gres_ns = NULL;
