...
/* 
	In the metastack 2.3.0 version, the survival time of the acctg_apptype thread was controlled through a counter. 
	However, this method was not suitable for controlling the overall survival time of the thread. Therefore, in 
	the 3.0.0 version, the thread survival time was controlled using a timer instead.
*/
bool apptype_thread_running = false;
pthread_mutex_t apptype_thread_mutex = PTHREAD_MUTEX_INITIALIZER;
#endif
...
{
	/*
	 * A reasonable minimum sampling granularity threshold is dynamically calculated based on the aggregation depth, 
	 * which is used for subsequent frequency adjustment and anomaly detection timer allocation. The larger this value is, 
	 * the coarser the aggregation granularity becomes, resulting in a lower sampling frequency to better accommodate system load.
	 */
	...
	if((acct_gather_profile_timer[type].freq > 0) && tmp > 0) {
		if(step_rank->timer  <= (acct_gather_profile_timer[type].freq) ) {
			step_rank->timer = ((acct_gather_profile_timer[type].freq + 59) / 60) * 60; // Round up to 60
		...
		if(step_rank->timer  <= (acct_gather_profile_timer[type].freq) ) 
			step_rank->timer = ((acct_gather_profile_timer[type].freq + 59) / 60) * 60; // Round up to 60s
		/* If the tree has only one layer, set the sampling frequency to 3, 
		 * which complies with the Shannon sampling theorem. 
		 */
		acct_gather_profile_timer[type].freq = step_rank->timer / 3;
	}
}
...
#ifdef __METASTACK_NEW_APPTYPE_RECOGNITION
				/*
					Check the data once at the beginning of the operation step 
					(even if there is a high probability that valid information 
...
#ifdef __METASTACK_NEW_APPTYPE_RECOGNITION
				/*
					Avoid printing invalid logs related to application identification
				*/
