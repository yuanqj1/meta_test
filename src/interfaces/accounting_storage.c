...
typedef struct slurm_acct_storage_ops {
	...
	int  (*commit)             (void *db_conn, bool commit, bool has_registered_lock);
#else
	...
	int (*fix_borrowaway_nodes)(void *db_conn, uint32_t uid, List nodes);
...
#ifdef __METASTACK_NEW_AUTO_SUPPLEMENT_AVAIL_NODES
	"acct_storage_p_fix_borrowaway_nodes",
...
#ifdef __METASTACK_BUG_CTLD_RESTART_POLL_HANG_FIX
extern int acct_storage_g_commit(void *db_conn, bool commit, bool has_registered_lock)
#else
...
#ifdef __METASTACK_BUG_CTLD_RESTART_POLL_HANG_FIX
	return (*(ops.commit))(db_conn, commit, has_registered_lock);
#else
...
#ifdef __METASTACK_NEW_AUTO_SUPPLEMENT_AVAIL_NODES
extern int acct_storage_g_fix_borrowaway_nodes(void *db_conn,
