...
scontrol_load_job(job_info_msg_t ** job_buffer_pptr, uint32_t job_id)
{
	...
	if (old_job_info_ptr) {
		...
		} else if (params.sctl_uid) {
			error_code = slurm_load_job_user(&job_info_ptr, params.sctl_uid, show_flags);
#endif
		} else {
...
#ifdef __METASTACK_OPT_SCONTROL_JOBUSER
/* return 0 if job should be printed */
static int _filter_job(job_info_t * job)
{
	int is_filter = 1;
	uint32_t *uuid;
	
	if (job->job_id == 0 || params.sctl_user_list == NULL) {
		return 0;
	}

	list_itr_t * itr = list_iterator_create(params.sctl_user_list);
	while ((uuid = list_next(itr))) {
		if (*uuid == job->user_id) {
			is_filter = 0;
			break;
		}
