...
#ifdef __METASTACK_NEW_GRES_DCU
  { "SALLOC_DCUS", 'U' },
  { "SALLOC_DCU_BIND", LONG_OPT_DCU_BIND },
  { "SALLOC_DCU_FREQ", LONG_OPT_DCU_FREQ },
  { "SALLOC_DCUS_PER_NODE", LONG_OPT_DCUS_PER_NODE },
...
#ifdef __METASTACK_NEW_GRES_DCU
	if ((opt.ntasks_per_dcu != NO_VAL) &&
	    (getenv("SLURM_NTASKS_PER_DCU") == NULL)) {
		setenvf(NULL, "SLURM_NTASKS_PER_DCU", "%d",
			opt.ntasks_per_dcu);
	}
#endif
#ifdef __METASTACK_NEW_GRES_NPU
