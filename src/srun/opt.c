...
static void remove_field(char *str, const char *key);
/*
...
static void remove_field(char *str, const char *key) {
	if(str == NULL || key == NULL)
		return; 
	char *pos = xstrcasestr(str, key);
	while (pos) {
		char *start = pos;
		char *end = xstrchr(pos, ',');
...
static void _process_abnormal_dete(){
	 ...
	 */
	if(opt.abnormal_dete && opt.acctg_freq) {
		if ( xstrcasestr(opt.abnormal_dete, "time_window=")) {
			remove_field(opt.acctg_freq, "time_window=");
		}
		if (xstrcasestr(opt.abnormal_dete, "avecpuutil=")) {
			remove_field(opt.acctg_freq, "avecpuutil=");
		}
	...
	if (opt.acctg_freq) {
		task = _get_int(opt.acctg_freq);
		if ((task == -1) && (sub_str = xstrcasestr(opt.acctg_freq, "task=")))
			task = _get_int(sub_str + 5);
	} else if (slurm_conf.job_acct_gather_freq) {
		task = _get_int(slurm_conf.job_acct_gather_freq);
		if ((task == -1) && (sub_str = xstrcasestr(slurm_conf.job_acct_gather_freq, "task=")))
			task = _get_int(sub_str + 5);
	}
	 ...
	 */
	if ((opt.acctg_freq != NULL) && (strlen(opt.acctg_freq) > 0))
...
static slurm_opt_t *_opt_copy(void)
{
	...
	opt.dcu_bind = NULL;		/* Moved by memcpy */
	opt.dcu_freq = NULL;		/* Moved by memcpy */
	opt.dcus = NULL;		/* Moved by memcpy */
	opt.dcus_per_node = NULL;	/* Moved by memcpy */
...
extern int initialize_and_process_args(int argc, char **argv, int *argc_off)
{
	...
	for (i = i_first; i <= i_last; i++) {
		...
		*/
		if ((opt.apptype == NULL || xstrcmp(opt.apptype, "unset") != 0) && opt.argc >= 1) {
		...
		if (opt.abnormal_dete != NULL) {
			...
			debug("Concatenate the job-monitor and acctg-freq parameters : srun -> acctg_freq = %s" , opt.acctg_freq);
		}
#endif
		if (opt.verbose)
...
#ifdef __METASTACK_NEW_GRES_DCU
  { "SLURM_DCUS", 'U' },
  { "SLURM_DCU_BIND", LONG_OPT_DCU_BIND },
  { "SLURM_DCU_FREQ", LONG_OPT_DCU_FREQ },
  { "SLURM_DCUS_PER_NODE", LONG_OPT_DCUS_PER_NODE },
...
"            [--mem-per-gpu=MB]\n"
#ifdef __METASTACK_NEW_GRES_DCU
"            [--cpus-per-dcu=n] [--dcus=n] [--dcu-bind=...] [--dcu-freq=...]\n"
...
#ifdef __METASTACK_NEW_GRES_DCU
	printf("\n"
"DCU scheduling options:\n"
"      --cpus-per-dcu=n        number of CPUs required per allocated DCU\n"
"  -U, --dcus=n                count of DCUs required for the job\n"
"      --dcu-bind=...          task to dcu binding options\n"
"      --dcu-freq=...          frequency and voltage of DCUs\n"
"      --dcus-per-node=n       number of DCUs required per allocated node\n"
