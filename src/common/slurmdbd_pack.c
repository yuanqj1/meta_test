...
#ifdef __METASTACK_OPT_SACCT_COMMAND
		safe_unpackstr(&msg_ptr->command, buffer);
#endif
#ifdef __METASTACK_OPT_SACCT_OUTPUT
		safe_unpackstr(&msg_ptr->stdout, buffer);
		safe_unpackstr(&msg_ptr->stderr, buffer);
#endif
#ifdef __METASTACK_OPT_RESC_NODEDETAIL
		safe_unpackstr(&msg_ptr->resource_node_detail, buffer);
#endif
	} else if (rpc_version >= SLURM_23_02_PROTOCOL_VERSION) {
		safe_unpackstr(&msg_ptr->admin_comment, buffer);
		safe_unpack32(&msg_ptr->assoc_id, buffer);
...
static int _unpack_node_borrow_msg(dbd_node_state_msg_t **msg,
				  uint16_t rpc_version, buf_t *buffer)
{
	dbd_node_state_msg_t *msg_ptr = NULL;
	uint32_t uint32_tmp;
