...
struct job_record {
	...
	char *command;			/* pathname of scripts and args */
#endif
#ifdef __METASTACK_OPT_RESC_NODEDETAIL
	char *resource_node_detail;		/* cpus info of job each node*/
#endif
	...
	bool is_lock_init;
	pthread_mutex_t job_sched_lock;  /* Control that the same job can */
                                     /* only be scheduled in one scheduling thread*/
                                     /* at the same time */
#endif		
	...
	bool lic_main_schedule; /* true when job scheduled by the main scheduling thread */
#endif
	...
	bool repeat_job;	/* Sets whether the job is a duplicate job */
#endif
...
//	job_record_t *job_ptr;	/* Template job structure pointer */
	uint32_t job_id;		/* job ID */
	uint32_t state_reason;		/* reason job still pending or failed*/
	char *job_resource_str;	/* Job application resource records */
};
#endif
