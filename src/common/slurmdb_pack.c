				   ...
				   buf_t *buffer)
{
	...
	if (protocol_version >= META_3_0_PROTOCOL_VERSION) {
		...
		error("slurmdb_pack_user_cond: protocol_version "
				    ...
				    buf_t *buffer)
{
	...
	*object = object_ptr;
	...
	if (protocol_version >= META_3_0_PROTOCOL_VERSION) {
		...
		error("slurmdb_unpack_user_cond: protocol_version "
		      "%hu not supported", protocol_version);
	}
#endif
	...
	*object = object_ptr;
	...
	if (protocol_version >= META_3_2_PROTOCOL_VERSION) {
		...
		if (count > NO_VAL)  
			goto unpack_error;  
		if (count != NO_VAL) {  
			object_ptr->appname_list = list_create(xfree_ptr);  
			for (i = 0; i < count; i++) {  
				safe_unpackstr_xmalloc(&tmp_info, &uint32_tmp,  
						       buffer);  
				list_append(object_ptr->appname_list, tmp_info);  
			}  
		}  
		...
		if (count > NO_VAL)  
			goto unpack_error;  
		if (count != NO_VAL) {  
			object_ptr->appversion_list = list_create(xfree_ptr);  
			for (i = 0; i < count; i++) {  
				safe_unpackstr_xmalloc(&tmp_info, &uint32_tmp,  
						       buffer);  
...
//		packstr(command, buffer);
//		xfree(command);
		packstr(job->command, buffer);
#endif
...
#ifdef __METASTACK_OPT_SACCT_COMMAND
		safe_unpackstr(&job_ptr->command, buffer);
#endif
#ifdef __METASTACK_OPT_RESC_NODEDETAIL
...
#endif
#ifdef __METASTACK_OPT_SACCT_OUTPUT
		safe_unpackstr(&job_ptr->stdout, buffer);
		safe_unpackstr(&job_ptr->stderr, buffer);
#endif
#ifdef __METASTACK_OPT_RESC_NODEDETAIL
		safe_unpackstr(&job_ptr->resource_node_detail, buffer);
#endif
