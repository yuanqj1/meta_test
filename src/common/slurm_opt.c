...
COMMON_STRING_OPTION_GET(submit_line);
...
COMMON_INT_OPTION(cpus_per_dcu, "--cpus-per-dcu");
static slurm_cli_opt_t slurm_opt_cpus_per_dcu = {
	.name = "cpus-per-dcu",
	.has_arg = required_argument,
	.val = LONG_OPT_CPUS_PER_DCU,
	.set_func = arg_set_cpus_per_dcu,
	.set_func_data = arg_set_data_cpus_per_dcu,
	.get_func = arg_get_cpus_per_dcu,
	.reset_func = arg_reset_cpus_per_dcu,
	.reset_each_pass = true,
};
#endif
...
#ifdef __METASTACK_NEW_GRES_DCU
static int arg_set_dcu_bind(slurm_opt_t *opt, const char *arg)
{
	xfree(opt->dcu_bind);
	xfree(opt->tres_bind);
	opt->dcu_bind = xstrdup(arg);
	xstrfmtcat(opt->tres_bind, "gres/dcu:%s", opt->dcu_bind);
	if (tres_bind_verify_cmdline(opt->tres_bind)) {
		error("Invalid --dcu-bind argument: %s", opt->dcu_bind);
		xfree(opt->dcu_bind);
		xfree(opt->tres_bind);
		return SLURM_ERROR;
	}
...
static int arg_set_data_dcu_bind(slurm_opt_t *opt, const data_t *arg,
				 data_t *errors)
{
	int rc;
	char *str = NULL;
	...
	if ((rc = data_get_string_converted(arg, &str)))
		...
		xfree(opt->dcu_bind);
		xfree(opt->tres_bind);
		opt->dcu_bind = xstrdup(str);
		xstrfmtcat(opt->tres_bind, "gres/dcu:%s", opt->dcu_bind);
		if (tres_bind_verify_cmdline(opt->tres_bind)) {
			rc = SLURM_ERROR;
			ADD_DATA_ERROR("Invalid --dcu-bind argument", rc);
			xfree(opt->dcu_bind);
			xfree(opt->tres_bind);
		}
	}
...
static void arg_reset_dcu_bind(slurm_opt_t *opt)
{
	xfree(opt->dcu_bind);
	xfree(opt->tres_bind);
}
COMMON_STRING_OPTION_GET(dcu_bind);
static slurm_cli_opt_t slurm_opt_dcu_bind = {
	.name = "dcu-bind",
	.has_arg = required_argument,
	.val = LONG_OPT_DCU_BIND,
	.set_func = arg_set_dcu_bind,
	.set_func_data = arg_set_data_dcu_bind,
	.get_func = arg_get_dcu_bind,
	.reset_func = arg_reset_dcu_bind,
	.reset_each_pass = true,
};

static int arg_set_dcu_freq(slurm_opt_t *opt, const char *arg)
{
	xfree(opt->dcu_freq);
	xfree(opt->tres_freq);
	opt->dcu_freq = xstrdup(arg);
	xstrfmtcat(opt->tres_freq, "dcu:%s", opt->dcu_freq);
	if (tres_freq_verify_cmdline(opt->tres_freq)) {
		error("Invalid --dcu-freq argument: %s", opt->tres_freq);
		xfree(opt->dcu_bind);
		xfree(opt->tres_bind);
		return SLURM_ERROR;
	}
...
static int arg_set_data_dcu_freq(slurm_opt_t *opt, const data_t *arg,
				 data_t *errors)
{
	int rc;
	char *str = NULL;
	...
	if ((rc = data_get_string_converted(arg, &str)))
		...
		xfree(opt->dcu_freq);
		xfree(opt->tres_freq);
		opt->dcu_freq = xstrdup(str);
		xstrfmtcat(opt->tres_freq, "dcu:%s", opt->dcu_freq);
		if (tres_freq_verify_cmdline(opt->tres_freq)) {
			rc = SLURM_ERROR;
			ADD_DATA_ERROR("Invalid --dcu-freq argument", rc);
			xfree(opt->dcu_freq);
			xfree(opt->tres_freq);
		}
	}
...
static void arg_reset_dcu_freq(slurm_opt_t *opt)
{
	xfree(opt->dcu_freq);
	xfree(opt->tres_freq);
}
COMMON_STRING_OPTION_GET(dcu_freq);
static slurm_cli_opt_t slurm_opt_dcu_freq = {
	.name = "dcu-freq",
	.has_arg = required_argument,
	.val = LONG_OPT_DCU_FREQ,
	.set_func = arg_set_dcu_freq,
	.set_func_data = arg_set_data_dcu_freq,
	.get_func = arg_get_dcu_freq,
	.reset_func = arg_reset_dcu_freq,
	.reset_each_pass = true,
};

COMMON_STRING_OPTION(dcus);
static slurm_cli_opt_t slurm_opt_dcus = {
	.name = "dcus",
	.has_arg = required_argument,
	.val = 'U',
	.set_func = arg_set_dcus,
	.set_func_data = arg_set_data_dcus,
	.get_func = arg_get_dcus,
	.reset_func = arg_reset_dcus,
	.reset_each_pass = true,
};

COMMON_STRING_OPTION(dcus_per_node);
static slurm_cli_opt_t slurm_opt_dcus_per_node = {
	.name = "dcus-per-node",
	.has_arg = required_argument,
	.val = LONG_OPT_DCUS_PER_NODE,
	.set_func = arg_set_dcus_per_node,
	.set_func_data = arg_set_data_dcus_per_node,
	.get_func = arg_get_dcus_per_node,
	.reset_func = arg_reset_dcus_per_node,
	.reset_each_pass = true,
};

COMMON_STRING_OPTION(dcus_per_socket);
static slurm_cli_opt_t slurm_opt_dcus_per_socket = {
	.name = "dcus-per-socket",
	.has_arg = required_argument,
	.val = LONG_OPT_DCUS_PER_SOCKET,
	.set_func = arg_set_dcus_per_socket,
	.set_func_data = arg_set_data_dcus_per_socket,
	.get_func = arg_get_dcus_per_socket,
	.reset_func = arg_reset_dcus_per_socket,
	.reset_each_pass = true,
};

COMMON_STRING_OPTION(dcus_per_task);
static slurm_cli_opt_t slurm_opt_dcus_per_task = {
	.name = "dcus-per-task",
	.has_arg = required_argument,
	.val = LONG_OPT_DCUS_PER_TASK,
	.set_func = arg_set_dcus_per_task,
	.set_func_data = arg_set_data_dcus_per_task,
	.get_func = arg_get_dcus_per_task,
	.reset_func = arg_reset_dcus_per_task,
	.reset_each_pass = true,
};
#endif
...
static int arg_set_npu_bind(slurm_opt_t *opt, const char *arg)
{
	...
	xstrfmtcat(opt->tres_bind, "gres/npu:%s", opt->npu_bind);
	if (tres_bind_verify_cmdline(opt->tres_bind)) {
		error("Invalid --npu-bind argument: %s", opt->npu_bind);
		xfree(opt->npu_bind);
		xfree(opt->tres_bind);
		return SLURM_ERROR;
	}
...
#ifdef __METASTACK_NEW_GRES_DCU
COMMON_MBYTES_OPTION(mem_per_dcu, --mem-per-dcu);
static slurm_cli_opt_t slurm_opt_mem_per_dcu = {
	.name = "mem-per-dcu",
	.has_arg = required_argument,
	.val = LONG_OPT_MEM_PER_DCU,
	.set_func = arg_set_mem_per_dcu,
	.set_func_data = arg_set_data_mem_per_dcu,
	.get_func = arg_get_mem_per_dcu,
	.reset_func = arg_reset_mem_per_dcu,
	.reset_each_pass = true,
};
#endif
...
#ifdef __METASTACK_NEW_GRES_DCU
COMMON_INT_OPTION_SET(ntasks_per_dcu, "--ntasks-per-dcu");
COMMON_INT_OPTION_SET_DATA(ntasks_per_dcu);
COMMON_INT_OPTION_GET(ntasks_per_dcu);
COMMON_OPTION_RESET(ntasks_per_dcu, NO_VAL);
static slurm_cli_opt_t slurm_opt_ntasks_per_dcu = {
	.name = "ntasks-per-dcu",
	.has_arg = required_argument,
	.val = LONG_OPT_NTASKSPERDCU,
	.set_func = arg_set_ntasks_per_dcu,
...
#ifdef __METASTACK_NEW_GRES_DCU
		else if (slurm_option_isset(opt, "mem-per-dcu"))
			info("Configured SelectTypeParameters doesn't treat memory as a consumable resource. In this case value of --mem-per-dcu is ignored.");
#endif
#ifdef __METASTACK_NEW_GRES_NPU
		else if (slurm_option_isset(opt, "mem-per-npu"))
			info("Configured SelectTypeParameters doesn't treat memory as a consumable resource. In this case value of --mem-per-npu is ignored.");
#endif
	}
}
...
static void _validate_ntasks_per_gpu(slurm_opt_t *opt)
{
...
#ifdef __METASTACK_NEW_GRES_DCU
	if (dcu && tres) {
		if (opt->ntasks_per_dcu != opt->ntasks_per_tres)
			fatal("Inconsistent values set to --ntasks-per-dcu=%d and --ntasks-per-tres=%d ",
			      opt->ntasks_per_dcu,
			      opt->ntasks_per_tres);
	} else if (dcu && tres_env) {
		if (opt->verbose)
			info("Ignoring SLURM_NTASKS_PER_TRES since --ntasks-per-dcu given as command line option");
		slurm_option_reset(opt, "ntasks-per-tres");
	} else if (tres && dcu_env) {
		if (opt->verbose)
			info("Ignoring SLURM_NTASKS_PER_DCU since --ntasks-per-tres given as command line option");
		slurm_option_reset(opt, "ntasks-per-dcu");
	} else if (dcu_env && tres_env) {
		if (opt->ntasks_per_dcu != opt->ntasks_per_tres)
			fatal("Inconsistent values set by environment variables SLURM_NTASKS_PER_DCU=%d and SLURM_NTASKS_PER_TRES=%d ",
			      opt->ntasks_per_dcu,
			      opt->ntasks_per_tres);
	}

	if (gpu && dcu) {
		fatal("--ntasks-per-gpu and --ntasks-per-dcu are mutually exclusive");
	} else if (gpu && dcu_env) {
		fatal("--ntasks-per-dcu and SLURM_NTASKS_PER_DCU are mutually exclusive");
	} else if (gpu_env && dcu) {
		fatal("SLURM_NTASKS_PER_GPU and --ntasks-per-dcu are mutually exclusive");
	} else if (gpu_env && dcu_env) {
		fatal("SLURM_NTASKS_PER_GPU and SLURM_NTASKS_PER_DCU are mutually exclusive");
	}

	if (dcu || dcu_env) {
		if (!opt->dcus && !xstrstr(opt->gres, "dcu") && !dcu_env)
	...
	if (slurm_option_set_by_cli(opt, LONG_OPT_TRES_PER_TASK))
		fatal("--tres-per-task is mutually exclusive with --ntasks-per-gpu and SLURM_NTASKS_PER_GPU and --ntasks-per-dcu and SLURM_NTASKS_PER_DCU and --ntasks-per-npu and SLURM_NTASKS_PER_NPU");

	if (slurm_option_set_by_env(opt, LONG_OPT_TRES_PER_TASK))
		fatal("SLURM_TRES_PER_TASK is mutually exclusive with --ntasks-per-gpu and SLURM_NTASKS_PER_GPU and --ntasks-per-dcu and SLURM_NTASKS_PER_DCU and --ntasks-per-npu and SLURM_NTASKS_PER_NPU");
#endif
	...
	if (slurm_option_set_by_env(opt, LONG_OPT_DCUS_PER_TASK))
		fatal("SLURM_DCUS_PER_TASK is mutually exclusive with --ntasks-per-dcu and SLURM_NTASKS_PER_DCU");
	
	if (slurm_option_set_by_cli(opt, LONG_OPT_DCUS_PER_SOCKET))
		fatal("--dcus-per-socket is mutually exclusive with --ntasks-per-dcu and SLURM_NTASKS_PER_DCU");
...
#ifdef __METASTACK_NEW_GRES_DCU
static bool _get_dcu_cnt_and_str(slurm_opt_t *opt, int *dcu_cnt, char **dcu_str)
{
	char *num_str = NULL, sep_char;

	if (!opt->dcus_per_task)
		return false;

	xstrcat(*dcu_str, "gres/dcu");

	if ((num_str = xstrstr(opt->dcus_per_task, ":")))
		sep_char = ':';
	else if ((num_str = xstrstr(opt->dcus_per_task, "=")))
		sep_char = '=';

	if (num_str) { /* Has type string */
		*num_str = '\0';
		/* Add type string to dcu_str */
		xstrfmtcat(*dcu_str, ":%s", opt->dcus_per_task);
		*num_str = sep_char;
		num_str += 1;
	} else {
		num_str = opt->dcus_per_task;
	}

	if (dcu_cnt)
		(*dcu_cnt) = strtol(num_str, NULL, 10);
...
#ifdef __METASTACK_NEW_GRES_NPU
static bool _get_npu_cnt_and_str(slurm_opt_t *opt, int *npu_cnt, char **npu_str)
{
	char *num_str = NULL, sep_char;
	...
	xstrcat(*npu_str, "gres/npu");

	if ((num_str = xstrstr(opt->npus_per_task, ":")))
		sep_char = ':';
	else if ((num_str = xstrstr(opt->npus_per_task, "=")))
		sep_char = '=';

	if (num_str) { /* Has type string */
		*num_str = '\0';
		/* Add type string to npu_str */
		xstrfmtcat(*npu_str, ":%s", opt->npus_per_task);
		*num_str = sep_char;
		num_str += 1;
	} else {
		num_str = opt->npus_per_task;
	}

	if (npu_cnt)
		(*npu_cnt) = strtol(num_str, NULL, 10);
...
static void _set_tres_per_task_from_sibling_opt(slurm_opt_t *opt, int optval)
{
...
#ifdef __METASTACK_NEW_GRES_DCU
	} else if (optval == LONG_OPT_DCUS_PER_TASK) {
		set = _get_dcu_cnt_and_str(opt, &cnt, &str);
		env_variable = "SLURM_DCUS_PER_TASK";
#endif
#ifdef __METASTACK_NEW_GRES_NPU
 	} else if (optval == LONG_OPT_NPUS_PER_TASK) {
		set = _get_npu_cnt_and_str(opt, &cnt, &str);
		env_variable = "SLURM_NPUS_PER_TASK";
#endif
	} else if (optval == 'c') {
	...
	if (optval == LONG_OPT_GPUS_PER_TASK) {
		...
		opt->dcus_per_task = opt_in_tpt_ptr;
#endif
#ifdef __METASTACK_NEW_GRES_NPU
	} else if (optval == LONG_OPT_NPUS_PER_TASK) {
		opt->npus_per_task = opt_in_tpt_ptr;
#endif
	} else if (optval == 'c') {
...
#ifdef __METASTACK_NEW_GRES_DCU
	_set_tres_per_task_from_sibling_opt(opt, LONG_OPT_DCUS_PER_TASK);
#endif
#ifdef __METASTACK_NEW_GRES_NPU
	_set_tres_per_task_from_sibling_opt(opt, LONG_OPT_NPUS_PER_TASK);
#endif
...
static void _validate_cpus_per_tres(slurm_opt_t *opt)
{
...
#ifdef __METASTACK_NEW_GRES_DCU
	if ((cpt_set_by_cli &&
	     slurm_option_set_by_cli(opt, LONG_OPT_CPUS_PER_DCU)) ||
	    (cpt_set_by_env &&
	     slurm_option_set_by_env(opt, LONG_OPT_CPUS_PER_DCU))) {
		fatal("--cpus-per-task, --tres-per-task=cpu:#, and --cpus-per-dcu are mutually exclusive");
	}
#endif
#ifdef __METASTACK_NEW_GRES_NPU
	if ((cpt_set_by_cli &&
	     slurm_option_set_by_cli(opt, LONG_OPT_CPUS_PER_NPU)) ||
	    (cpt_set_by_env &&
	     slurm_option_set_by_env(opt, LONG_OPT_CPUS_PER_NPU))) {
		fatal("--cpus-per-task, --tres-per-task=cpu:#, and --cpus-per-npu are mutually exclusive");
	}
#endif
	...
	if (cpt_set_by_cli &&
		...
		slurm_option_update_tres_per_task(opt->cpus_per_task, "cpu",
						  &opt->tres_per_task);
#ifdef __METASTACK_NEW_GRES_DCU
	} else if (cpt_set_by_cli &&
		   slurm_option_set_by_env(opt, LONG_OPT_CPUS_PER_DCU)) {
		if (opt->verbose) {
			char *env_str = NULL;

			if (opt->salloc_opt)
				env_str = "SALLOC_CPUS_PER_DCU";
			else if (opt->sbatch_opt)
				env_str = "SBATCH_CPUS_PER_DCU";
			else /* opt->srun_opt */
				env_str = "SLURM_CPUS_PER_DCU";
			info("Ignoring %s since --cpus-per-task or --tres-per-task=cpu:# given as command line option",
				env_str);
		}
		slurm_option_reset(opt, "cpus-per-dcu");
	} else if (slurm_option_set_by_cli(opt,	LONG_OPT_CPUS_PER_DCU) &&
		    cpt_set_by_env) {
		if (opt->verbose) {
			info("Ignoring cpus_per_task from the environment since --cpus-per-dcu was given as a command line option");
		}
		slurm_option_reset(opt, "cpus-per-task");
		/* Also clear cpu:# from tres-per-task */
		slurm_option_update_tres_per_task(opt->cpus_per_task, "cpu",
		...
		if (opt->verbose) {
			...
			if (opt->salloc_opt)
				env_str = "SALLOC_CPUS_PER_NPU";
			else if (opt->sbatch_opt)
				env_str = "SBATCH_CPUS_PER_NPU";
			else /* opt->srun_opt */
				env_str = "SLURM_CPUS_PER_NPU";
			info("Ignoring %s since --cpus-per-task or --tres-per-task=cpu:# given as command line option",
				env_str);
		}
		slurm_option_reset(opt, "cpus-per-npu");
	} else if (slurm_option_set_by_cli(opt,	LONG_OPT_CPUS_PER_NPU) &&
		    cpt_set_by_env) {
		if (opt->verbose) {
			info("Ignoring cpus_per_task from the environment since --cpus-per-dcu was given as a command line option");
		}
...
#ifdef __METASTACK_NEW_GRES_DCU
	xfmt_tres(&job_desc->tres_per_job, "gres/dcu", opt_local->dcus);
	xfmt_tres(&job_desc->tres_per_node, "gres/dcu",
...
#ifdef __METASTACK_NEW_GRES_DCU
	xfmt_tres(&job_desc->tres_per_socket, "gres/dcu",
