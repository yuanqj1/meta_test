...
// __METASTACK_OPT_MSG_OUTPUT_MULTI_LANG
...
extern char *get_err_msg(char *conf_path, int err_code) {
	...
	if (loc != (locale_t)0) {
		...
		if (codeset && (xstrcasecmp(codeset, "UTF-8") == 0 || xstrcasecmp(codeset, "UTF8") == 0)) {
			is_utf8 = true;
		}
	...
	if (!is_utf8 && xstrncasecmp(target_lang, "en", 2) != 0) {
	...
	while (fgets(buf, sizeof(buf), fp)) {
		...
		if (!token_code || !token_lang)
			continue;

		
		for (p = token_code; *p; p++) {
		...
		if (!token_val) {
			if (atoi(token_code) == err_code) {
				char *newline = strpbrk(token_lang, "\n\r");
				if (newline)
					*newline = '\0';
		
				xfree(legacy_msg);
				legacy_msg = xstrdup(token_lang);
			}
			continue;
		}

		// new format: "error_code=language=message_string"
		if (atoi(token_code) != err_code)
			continue;

		// 1. Perfect match: Target language found
		if (xstrcmp(token_lang, target_lang) == 0) {
			err_msg = xstrdup(token_val);
			break; 
		} else if (xstrcmp(token_lang, "en_US") == 0) {
...
extern int slurm_send_node_msg1(int fd, slurm_msg_t *msg)
{
	...
	if (msg->conn) {
		...
		rc = slurm_persist_send_msg1(msg->conn, buffer);
		FREE_NULL_BUFFER(buffer);
			       ...
			       slurm_msg_t *resp, int timeout)
{
	...
	if (slurm_send_node_msg1(fd, req) < 0)
		return -1;
	 ...
	 */
	if (slurm_receive_msg1(fd, resp, timeout))
		return -1;
...
tryagain:
	...
	if (!rc && (response_msg->msg_type == RESPONSE_SLURM_RC) &&
		...
		if (tmp_retry_max_period) {
			...
			if(ratelimited > retry_max_period){
				ratelimited = retry_max_period;
			}
