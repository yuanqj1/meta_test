...
int setup_env(env_t *env, bool preserve_env)
{
	...
	if (env->ntasks_per_dcu &&
	    setenvf(&env->env, "SLURM_NTASKS_PER_DCU", "%d",
		    env->ntasks_per_dcu)) {
		error("Unable to set SLURM_NTASKS_PER_DCU");
		rc = SLURM_ERROR;
	}
#endif
	...
	if (env->app_name) {
		if (setenvf(&env->env, "SLURM_JOB_APP_SOURCE", "%s",
			    app_source_to_str(env->app_source))) {
...
#ifdef __METASTACK_NEW_GRES_DCU
	if (opt->cpus_per_dcu) {
		env_array_overwrite_het_fmt(dest, "SLURM_CPUS_PER_DCU",
					    het_job_offset, "%d",
					    opt->cpus_per_dcu);
	}
	if (opt->dcus) {
		env_array_overwrite_het_fmt(dest, "SLURM_DCUS",
					    het_job_offset, "%s",
					    opt->dcus);
	}
	if (opt->dcu_freq) {
		env_array_overwrite_het_fmt(dest, "SLURM_DCU_FREQ",
					    het_job_offset, "%s",
					    opt->dcu_freq);
	}
	if (opt->dcus_per_node) {
		env_array_overwrite_het_fmt(dest, "SLURM_DCUS_PER_NODE",
					    het_job_offset, "%s",
					    opt->dcus_per_node);
	}
	if (opt->dcus_per_socket) {
		env_array_overwrite_het_fmt(dest, "SLURM_DCUS_PER_SOCKET",
					    het_job_offset, "%s",
					    opt->dcus_per_socket);
	}
